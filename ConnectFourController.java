...
import View.ConnectFourJframe;
	...
	public class ConnectFourController implements ActionListener {
		...
		private final Game game; //Nytt
		
		private final Player player; //Nytt
		 ...
		 * @param model
		 */
		public ConnectFourController(final ConnectFourJframe view, final ConnectFourModel model, final Game game, final Player player) {
			this.view = view;
			this.model = model;
			this.game = game; //nytt
			this.player = player; //Nytt
			...
			JTextField playerName = new HintTextField("Player 1, write name here");
			...
			*/
			...
			JTextField p = new HintTextField("Enter name here");
			p.setBounds(6, 0, 134, 28);
			this.view.getContentPane().add(p);
			p.setColumns(10);
			this.player.setName(p.getText());
			p.setEditable(false);
			
			JTextField p1 = new HintTextField("Enter name here");
			p1.setBounds(650, 0, 134, 28);
			this.view.getContentPane().add(p);
			p1.setColumns(10);
			this.player.setName(p1.getText());
			p1.setEditable(false);
			
			JLabel player1 = new JLabel("Player 1");
			player1.setBounds(10, 6, 61, 16);
			this.view.getContentPane().add(player1);
			
			JLabel player2 = new JLabel("Player 2");
			player2.setBounds(750, 6, 61, 16);
			this.view.getContentPane().add(player2);
			
			JLabel yellow = new JLabel("Yellow");
			yellow.setBounds(120, 6, 61, 16);
			this.view.getContentPane().add(yellow);
			
			JTextField red = new HintTextField("Enter name here");
			
			red.setBounds(650, 6, 61, 16);
			this.view.getContentPane().add(red);
			
			/*JButton btnSelectPlayerName = new JButton("Save");
			btnSelectPlayerName.setBounds(285, 6, 152, 29);
			this.view.getContentPane().add(btnSelectPlayerName);
			/*
			//new
			btnSelectPlayerName.addActionListener(new ActionListener(){
				@Override
				public void actionPerformed(ActionEvent e){
					view.setPlayerName(playerName.getText().toString());
